...
def post_comment():
        ...
        access_token = result['commenter_id']
        commenter_id = jwt.decode(access_token, app.config['SECRET_KEY'], algorithms=['HS256'])['_id']
...
def user_login() : 
    ...
    if account:
        if check_password_hash(account['password'], pw):
            ...
            return jsonify({'result': 'success', 'access-token': token.decode('utf-8')})
